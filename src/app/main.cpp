...
int main(int argc, char *argv[]) {
  try {
    ...
    bool hi = true;
    /*eventDispatcher->subscribe(EventType::PG_Key_Down_D, [&](const Event&
    e){dst.x += 5;}); eventDispatcher->subscribe(EventType::PG_Key_Down_A,
    [&](const Event& e){dst.x -= 5;});
    eventDispatcher->subscribe(EventType::PG_Key_Down_S, [&](const Event&
    e){dst.y += 5;}); eventDispatcher->subscribe(EventType::PG_Key_Down_W,
    [&](const Event& e){dst.y -= 5;});*/
