...
namespace Utils
{
    ...
    public:
        ...
        {
            if (dll_is_load) { throw std::runtime_error("DLL already loaded: " + dllPath); }
            
            hDll = LoadLibraryA(dllPath.c_str());
            if (!hDll) { throw std::runtime_error("Failed to load DLL: " + dllPath); }
    
            createFunc = reinterpret_cast<Type* (*)()>(GetProcAddress(hDll, createName.c_str()));
            if (!createFunc)
            {
                unload();
            ...
            destroyFunc = reinterpret_cast<void (*)(Type*)>(GetProcAddress(hDll, destroyName.c_str()));
            if (!destroyFunc)
            {
                unload();
